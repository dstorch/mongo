...
void IndexScanStage::open(bool reOpen) {
    ...
    if (_open) {
        ...
        tassert(5071007, "IndexScanStage is open but _coll is not held", static_cast<bool>(_coll));
        tassert(
        ...
        if (!_coll) {
            ...
            tassert(5071010,
                    "IndexScanStage is not open but have _cursor",
                    !static_cast<bool>(_cursor));
            restoreCollection(_opCtx, _collName, _collUuid, _lockAcquisitionCallback, _coll);
        }
    }
