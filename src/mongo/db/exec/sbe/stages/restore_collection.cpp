...
#include "mongo/db/exec/sbe/stages/restore_collection.h"
...
NamespaceString acquireCollection(OperationContext* opCtx,
    ...
    tassert(5071012, "cannot restore 'coll' if already held", !static_cast<bool>(coll));
...
void restoreCollection(OperationContext* opCtx,
                       ...
                       LockAcquisitionCallback lockAcquisitionCallback,
                       boost::optional<AutoGetCollectionForReadMaybeLockFree>& coll) {
    tassert(5071011, "cannot restore 'coll' if already held", !static_cast<bool>(coll));
