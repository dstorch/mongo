...
void ScanStage::open(bool reOpen) {
    ...
    if (_open) {
        ...
        tassert(5071002, "ScanStage is open but _coll is not held", static_cast<bool>(_coll));
        ...
        if (!_coll) {
            ...
            tassert(5071005, "ScanStage is not open but have _cursor", !static_cast<bool>(_cursor));
            restoreCollection(_opCtx, _collName, _collUuid, _lockAcquisitionCallback, _coll);
        }
    }

    // TODO: this is currently used only to wait for oplog entries to become visible, so we may want
    // to consider to move this logic into storage API instead.
    if (_openCallback) {
...
void ParallelScanStage::open(bool reOpen) {
    ...
    if (!_coll) {
        ...
        // make some validity checks (the collection has not been dropped, renamed, etc.).
        tassert(
            5071008, "ParallelScanStage is not open but have _cursor", !static_cast<bool>(_cursor));
        restoreCollection(_opCtx, _collName, _collUuid, nullptr, _coll);
    }
