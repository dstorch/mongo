 ...
 * - CRS (flat or spherical)
 */
void encodeGeoNearMatchExpression(const GeoNearMatchExpression* tree, mongoutils::str::stream* os) {
...
Status CanonicalQuery::canonicalize(const string& ns,
    ...
    BSONObj emptyObj;
    Status parseStatus = LiteParsedQuery::make(
        ns, skip, limit, 0, query, proj, sort, hint, minObj, maxObj, snapshot, explain, &lpqRaw);
    if (!parseStatus.isOK()) {
        return parseStatus;
    }
...
std::string CanonicalQuery::toString() const {
    mongoutils::str::stream ss;
    ss << "ns=" << _pq->ns() << " limit=" << _pq->getNumToReturn() << " skip=" << _pq->getSkip()
       << '\n';
