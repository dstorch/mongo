...
TEST(CanonicalQueryTest, IsValidText) {
    ...
    ASSERT_OK(LiteParsedQuery::make(ns,
                                    0,
                                    0,
                                    0,
                                    fromjson("{}"),
 ...
 */
void testGetPlanCacheKey(const char* queryStr,
                         const char* sortStr,
                         const char* projStr,
...
TEST(PlanCacheTest, GetPlanCacheKey) {
    ...
    testGetPlanCacheKey("{}", "{}", "{a: {$elemMatch: {x: 2}}}", "an|{ $elemMatch: { x: 2 } }a");
...
TEST(PlanCacheTest, GetPlanCacheKeyGeoWithin) {
    // Legacy coordinates.
    auto_ptr<CanonicalQuery> cqLegacy(canonicalize(
        "{a: {$geoWithin: "
        "{$box: [[-180, -90], [180, 90]]}}}"));
    // GeoJSON coordinates.
    auto_ptr<CanonicalQuery> cqNew(canonicalize(
        "{a: {$geoWithin: "
...
TEST(PlanCacheTest, GetPlanCacheKeyGeoNear) {
    testGetPlanCacheKey("{a: {$near: [0,0], $maxDistance:0.3 }}", "{}", "{}", "gnanrfl");
    testGetPlanCacheKey("{a: {$nearSphere: [0,0], $maxDistance: 0.31 }}", "{}", "{}", "gnanssp");
    testGetPlanCacheKey(
