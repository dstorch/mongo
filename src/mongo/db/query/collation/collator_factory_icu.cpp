...
StatusWith<std::unique_ptr<CollatorInterface>> CollatorFactoryICU::makeFromBSON(
    ...
    // TODO: use ICU to normalize locale string.
    ...
    auto mongoICUCollator = stdx::make_unique<CollatorInterfaceICU>(
        std::move(parsedSpec.getValue()), std::move(icuCollator));
    std::unique_ptr<CollatorInterface> mongoCollator = std::move(mongoICUCollator);
