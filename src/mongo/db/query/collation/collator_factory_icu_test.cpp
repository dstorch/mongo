...
TEST(CollatorFactoryICUTest, FactoryMadeCollatorComparesStringsCorrectlyEnUS) {
    ...
    ASSERT_LT(collator.getValue()->compare(StringData("ab"), StringData("ba")), 0);
    ASSERT_GT(collator.getValue()->compare(StringData("ba"), StringData("ab")), 0);
    ASSERT_EQ(collator.getValue()->compare(StringData("ab"), StringData("ab")), 0);
}
