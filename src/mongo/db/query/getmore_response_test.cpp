...
namespace {
    ...
    TEST(GetMoreResponseTest, parseFromBSONFirstBatch) {
        StatusWith<GetMoreResponse> result = GetMoreResponse::parseFromBSON(
            BSON("cursor" << BSON("id" << CursorId(123) << "ns" << "db.coll" << "firstBatch" <<
    ...
    TEST(GetMoreResponseTest, parseFromBSONNextBatch) {
        StatusWith<GetMoreResponse> result = GetMoreResponse::parseFromBSON(
            BSON("cursor" << BSON("id" << CursorId(123) << "ns" << "db.coll" << "nextBatch" <<
                        BSON_ARRAY(BSON("_id" << 1) << BSON("_id" << 2))) <<
                 "ok" << 1));
        ASSERT_OK(result.getStatus());
    ...
    TEST(GetMoreResponseTest, parseFromBSONCursorIdZero) {
        StatusWith<GetMoreResponse> result = GetMoreResponse::parseFromBSON(
            BSON("cursor" << BSON("id" << CursorId(0) << "ns" << "db.coll" << "nextBatch" <<
                        BSON_ARRAY(BSON("_id" << 1) << BSON("_id" << 2))) <<
                 "ok" << 1));
        ASSERT_OK(result.getStatus());
    ...
    TEST(GetMoreResponseTest, parseFromBSONIdFieldMissing) {
        StatusWith<GetMoreResponse> result = GetMoreResponse::parseFromBSON(
            BSON("cursor" << BSON("ns" << "db.coll" << "nextBatch" <<
                        BSON_ARRAY(BSON("_id" << 1) << BSON("_id" << 2))) <<
                 "ok" << 1));
        ASSERT_NOT_OK(result.getStatus());
    }
    ...
    TEST(GetMoreResponseTest, parseFromBSONIdFieldWrongType) {
        StatusWith<GetMoreResponse> result = GetMoreResponse::parseFromBSON(
            BSON("cursor" << BSON("id" << "123" << "ns" << "db.coll" << "nextBatch" <<
                        BSON_ARRAY(BSON("_id" << 1) << BSON("_id" << 2))) <<
                 "ok" << 1));
        ASSERT_NOT_OK(result.getStatus());
    }
    ...
    TEST(GetMoreResponseTest, parseFromBSONBatchFieldMissing) {
        StatusWith<GetMoreResponse> result = GetMoreResponse::parseFromBSON(
            BSON("cursor" << BSON("id" << CursorId(123) << "ns" << "db.coll") << "ok" << 1));
        ASSERT_NOT_OK(result.getStatus());
    }

    TEST(GetMoreResponseTest, parseFromBSONFirstBatchFieldWrongType) {
        StatusWith<GetMoreResponse> result = GetMoreResponse::parseFromBSON(
            BSON("cursor" << BSON("id" << CursorId(123) << "ns" << "db.coll"
    ...
    TEST(GetMoreResponseTest, parseFromBSONHandleErrorResponse) {
        StatusWith<GetMoreResponse> result = GetMoreResponse::parseFromBSON(
            BSON("ok" << 0 << "code" << 123 << "errmsg" << "does not work"));
        ASSERT_NOT_OK(result.getStatus());
    ...
    TEST(GetMoreResponseTest, toBSON) {
        ...
        GetMoreResponse response("testdb.testcoll", CursorId(123), batch);
        BSONObj responseObj = response.toBSON();
        BSONObj expectedResponse = BSON("cursor"
            << BSON("id" << CursorId(123) << "ns" << "testdb.testcoll" << "nextBatch"
                << BSON_ARRAY(BSON("_id" << 1) << BSON("_id" << 2))) << "ok" << 1.0);
