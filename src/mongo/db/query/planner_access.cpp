...
std::vector<QuerySolutionNode*> QueryPlannerAccess::collapseEquivalentScans(
    ...
    for (size_t i = 1; i < ownedScans.size(); ++i) {
        if (scansAreEquivalent(collapsedScans.back(), ownedScans[i])) {
            ...
            // If there's no filter associated with a fetch node on 'collapseFrom', all we have
            // to do is clear the filter on the node that we are collapsing into.
            ...
            // If there's no filter associated with a fetch node on the back of the
            // 'collapsedScans' list, then there's nothing more to do.
            ...
            // OrMatchExpression.
            std::unique_ptr<OrMatchExpression> collapsedFilter(new OrMatchExpression());
...
QuerySolutionNode* QueryPlannerAccess::buildIndexedOr(const CanonicalQuery& query,
    ...
    // If any index scans are identical, then we collapse them into a single scan. This prevents
